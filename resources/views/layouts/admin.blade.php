        ...
        <div class="min-h-screen flex">
            ...
            <div class="w-64 bg-gray-900 text-white shadow-lg">
                <div class="p-2 border-b border-gray-700">
                    @php
                        $settings = \App\Models\CompanyProfile::first();
                        $hasLogo = $settings && $settings->logo && \Illuminate\Support\Facades\Storage::disk('public')->exists($settings->logo);
                        $logoPath = $hasLogo ? asset('storage/' . $settings->logo) : null;
                        $companyName = $settings && $settings->company_name ? $settings->company_name : 'Parampara';
                    @endphp
                    <a href="/" class="flex items-center justify-center">
                        @if ($hasLogo)
                            <img src="{{ $logoPath }}" alt="{{ $companyName }}" class="h-18 object-contain">
                        @else
                ...
                <nav class="mt-6 pb-32">
                    ...
                    <!-- Sales -->
                    <a href="{{ route('admin.sales.index') }}" class="block px-6 py-3 hover:bg-gray-800 transition {{ request()->routeIs('admin.sales.*') ? 'bg-gray-800 border-l-4 border-blue-500' : '' }}">
                        <span class="flex items-center">
                            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Sales Invoices (daily: cash + online) -->
                    <a href="{{ route('admin.sale-invoices.index') }}" class="block px-6 py-3 hover:bg-gray-800 transition {{ request()->routeIs('admin.sale-invoices.*') ? 'bg-gray-800 border-l-4 border-blue-500' : '' }}">
                        <span class="flex items-center">
                            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Expense Categories -->
                    <a href="{{ route('admin.expense-categories.index') }}" class="block px-6 py-3 hover:bg-gray-800 transition {{ request()->routeIs('admin.expense-categories.*') ? 'bg-gray-800 border-l-4 border-blue-500' : '' }}">
                        <span class="flex items-center">
                            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
