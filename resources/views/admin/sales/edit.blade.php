        ...
        <form action="{{ route('admin.sales.update', $sale) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div>
                    ...
                    <input type="date" id="sale_date" name="sale_date" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required value="{{ old('sale_date', $sale->sale_date->format('Y-m-d')) }}">
                    @error('sale_date')
                ...
                <div>
                    ...
                    <input type="text" id="seller_name" name="seller_name" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter seller name" value="{{ old('seller_name', $sale->seller_name) }}">
                    @error('seller_name')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="text" id="seller_contact_number" name="seller_contact_number" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter contact number" value="{{ old('seller_contact_number', $sale->seller_contact_number) }}">
                                    ...
                                    <td class="px-4 py-2">
                                        <select name="product_id[]" class="w-full px-3 py-2 border border-gray-300 rounded" required>
                                            ...
                                            @foreach ($products as $product)
                                                <option value="{{ $product->id }}" data-price="{{ $product->selling_price }}" {{ $item->product_id == $product->id ? 'selected' : '' }}>{{ $product->product_name }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td class="px-4 py-2">
            ...
            <div class="grid grid-cols-4 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" id="total_amount" name="total_amount" class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100 focus:outline-none" readonly value="{{ old('total_amount', number_format($sale->total_amount, 2, '.', '')) }}">
                </div>
                <div>
                    ...
                    <select id="payment_mode" name="payment_mode" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        @foreach (config('payment.sale_modes') as $value => $label)
                            <option value="{{ $value }}" {{ old('payment_mode', $displayPaymentMode) === $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('payment_mode')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
            ...
            <div class="mb-6">
                ...
                <textarea id="notes" name="notes" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('notes', $sale->notes) }}</textarea>
            </div>
...
document.querySelector('form[action="{{ route('admin.sales.update', $sale) }}"]').addEventListener('submit', function(event) {
