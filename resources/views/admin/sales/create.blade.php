...
<div class="max-w-6xl mx-auto">
    <div class="bg-white rounded-lg shadow p-6">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">Record New Sale</h2>
        ...
        <form action="{{ route('admin.sales.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div>
                    <label for="sale_date" class="block text-sm font-semibold text-gray-700 mb-2">Sale Date *</label>
                    <input type="date" id="sale_date" name="sale_date" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required value="{{ old('sale_date', date('Y-m-d')) }}">
                    @error('sale_date')
                ...
                <div>
                    ...
                    <input type="text" id="seller_name" name="seller_name" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter seller name" value="{{ old('seller_name') }}">
                    @error('seller_name')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="text" id="seller_contact_number" name="seller_contact_number" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter contact number" value="{{ old('seller_contact_number') }}">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full border border-gray-300">
                        <thead class="bg-gray-100">
                            <tr>
                                <th class="px-2 py-2 text-left text-sm font-semibold">Product</th>
                                <th class="px-2 py-2 text-left text-sm font-semibold">Quantity</th>
                                <th class="px-2 py-2 text-left text-sm font-semibold">Price</th>
                                <th class="px-2 py-2 text-left text-sm font-semibold">Total</th>
                                <th class="px-2 py-2 text-center text-sm font-semibold">Action</th>
                            </tr>
                        </thead>
            ...
            <div class="grid grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" id="total_amount" name="total_amount" class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100 focus:outline-none" readonly value="{{ old('total_amount', '0.00') }}">
                </div>
                <div>
                    ...
                    <select id="payment_mode" name="payment_mode" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="gpay" {{ old('payment_mode') === 'gpay' ? 'selected' : '' }}>G-pay</option>
                        <option value="mix" {{ old('payment_mode') === 'mix' ? 'selected' : '' }}>Mix (Cash & Online)</option>
                    </select>
                    @error('payment_mode')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
            ...
            <div class="flex gap-4">
                <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-6 rounded">Record Sale</button>
                <a href="{{ route('admin.sales.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-6 rounded">Cancel</a>
            </div>
        </form>
    </div>
</div>
...
document.querySelector('form[action="{{ route('admin.sales.store') }}"]').addEventListener('submit', function(event) {
