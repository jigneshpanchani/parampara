        ...
        <form action="{{ route('admin.sell-returns.store') }}" method="POST" class="bg-white rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" id="reason" name="reason" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g., Defective, Not as described" value="{{ old('reason') }}">
                    @error('reason')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
