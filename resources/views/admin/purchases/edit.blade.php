        ...
        <form action="{{ route('admin.purchases.update', $purchase) }}" method="POST">
            ...
            <div class="grid grid-cols-4 gap-4 mb-4">
                ...
                <div>
                    ...
                    @php $selectedBillType = old('bill_type', $purchase->bill_type); @endphp
                    <select id="bill_type" name="bill_type" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                        <option value="gst" {{ $selectedBillType === 'gst' ? 'selected' : '' }}>GST</option>
                        <option value="without_gst" {{ $selectedBillType === 'without_gst' ? 'selected' : '' }}>Without GST</option>
                    </select>
                    @error('bill_type')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
