    ...
    <table class="w-full">
        <thead class="bg-gray-100 border-b">
            <tr>
                ...
                <th class="px-6 py-3 text-right text-sm font-semibold text-gray-700">Pay Later</th>
                <th class="px-6 py-3 text-right text-sm font-semibold text-gray-700">Total Sales</th>
                <th class="px-6 py-3 text-right text-sm font-semibold text-gray-700">Return</th>
                <th class="px-6 py-3 text-right text-sm font-semibold text-gray-700">Expense</th>
                <th class="px-6 py-3 text-right text-sm font-semibold text-gray-700">Net Total</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($rows as $r)
                <tr class="border-b hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-sm text-right text-blue-600">{{ $r['cash'] > 0 ? '₹' . number_format($r['cash'], 2) : '—' }}</td>
                    <td class="px-6 py-4 text-sm text-right text-purple-600">{{ $r['online'] > 0 ? '₹' . number_format($r['online'], 2) : '—' }}</td>
                    <td class="px-6 py-4 text-sm text-right text-yellow-700">{{ $r['pay_later'] > 0 ? '₹' . number_format($r['pay_later'], 2) : '—' }}</td>
                    <td class="px-6 py-4 text-sm text-right text-gray-800">{{ $r['sales_amount'] > 0 ? '₹' . number_format($r['sales_amount'], 2) : '—' }}</td>
                    <td class="px-6 py-4 text-sm text-right text-orange-600">{{ $r['return'] > 0 ? '₹' . number_format($r['return'], 2) : '—' }}</td>
                    <td class="px-6 py-4 text-sm text-right text-red-600">{{ $r['expense'] > 0 ? '₹' . number_format($r['expense'], 2) : '—' }}</td>
                    <td class="px-6 py-4 text-sm text-right font-semibold {{ $r['net'] >= 0 ? 'text-green-700' : 'text-red-700' }}">₹{{ number_format($r['net'], 2) }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="8" class="px-6 py-4 text-center text-gray-600">No data found for the selected period.</td>
                </tr>
            @endforelse
        </tbody>
        @if(!empty($rows))
            <tfoot class="bg-gray-50 border-t-2 border-gray-300">
                <tr>
                    ...
                    <td class="px-6 py-3 text-sm text-right font-bold text-blue-700">₹{{ number_format($totals['cash'], 2) }}</td>
                    <td class="px-6 py-3 text-sm text-right font-bold text-purple-700">₹{{ number_format($totals['online'], 2) }}</td>
                    <td class="px-6 py-3 text-sm text-right font-bold text-yellow-700">₹{{ number_format($totals['pay_later'], 2) }}</td>
                    <td class="px-6 py-3 text-sm text-right font-bold text-gray-800">₹{{ number_format($totals['sales_amount'], 2) }}</td>
                    <td class="px-6 py-3 text-sm text-right font-bold text-orange-700">₹{{ number_format($totals['return'], 2) }}</td>
                    <td class="px-6 py-3 text-sm text-right font-bold text-red-700">₹{{ number_format($totals['expense'], 2) }}</td>
                    <td class="px-6 py-3 text-sm text-right font-bold {{ $totals['net'] >= 0 ? 'text-green-800' : 'text-red-800' }}">₹{{ number_format($totals['net'], 2) }}</td>
                </tr>
            </tfoot>
        @endif
    </table>
