...
<div class="max-w-6xl mx-auto">
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <form action="{{ route('admin.sells.store') }}" method="POST">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div>
                    <label for="sell_date" class="block text-sm font-semibold text-gray-700 mb-2">Sale Date *</label>
                    <input type="date" id="sell_date" name="sell_date" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required value="{{ old('sell_date', date('Y-m-d')) }}">
                    @error('sell_date')
...
document.querySelector('form[action="{{ route('admin.sells.store') }}"]').addEventListener('submit', function(event) {
