...
const roleData = usePage().props.role;
...
const roleHasPermission = (roleId) => {
    return props.roleHasPermissions.includes(roleId);
};
...
const props = defineProps(['data', 'roleHasPermissions']);
...
<template>
    <Head title="Update Role-Permission" />

    <AdminLayout>
        <div class="animate-top bg-white p-4 shadow sm:p-6 sm:rounded-lg border">
            <h2 class="text-2xl font-semibold leading-7 text-gray-900">Edit Role & Permission</h2>
            <form @submit.prevent="form.patch(route('roles.update', { role: form.id }))">
                <div class="border-b border-gray-900/10 pb-12">
                    <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-12">
                        ...
                        <div class="sm:col-span-3" v-for="(roles, name) in data">
                            ...
                            <div class="border border-t-0 rounded-b-lg">
                                <div v-for="(role, index) in roles" :key="index" class="flex justify-between items-center px-4 py-1 border-b last:border-b-0">
                                    ...
                                    <div class="flex justify-end p-1">
                                        <Checkbox
                                            :checked="roleHasPermission(role.id)"
